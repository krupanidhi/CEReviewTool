...
import {
  CheckCircle, XCircle, AlertTriangle, HelpCircle, Loader2, Download,
...
function SummaryCards({ summary }) {
  ...
  const rateColor = summary.agreementRate >= 80 ? 'text-green-400' : summary.agreementRate >= 50 ? 'text-yellow-400' : 'text-red-400'
  return (
    <div className="grid grid-cols-5 gap-3">
      {[
        { v: summary.totalQuestions, l: 'Questions', c: 'text-blue-400' },
        { v: summary.matchCount, l: 'Agree', c: 'text-green-400' },
        { v: summary.disagreeCount, l: 'Disagree', c: 'text-red-400' },
        { v: summary.uncertainCount, l: 'Uncertain', c: 'text-yellow-400' },
        { v: `${summary.agreementRate}%`, l: 'Agreement', c: rateColor },
      ].map((d, i) => (
...
function ResultsTable({ results, filter, setFilter, expanded, toggle }) {
  const filtered = filter === 'all' ? results : results.filter(r => r.match === filter)
  const counts = { agree: results.filter(r => r.match === 'agree').length, disagree: results.filter(r => r.match === 'disagree').length, uncertain: results.filter(r => r.match === 'uncertain').length }
  return (
    <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
        ...
        <div className="flex space-x-1.5">
          {['all', 'agree', 'disagree', 'uncertain'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${filter === f ? (f === 'agree' ? 'bg-green-500/30 text-green-400 ring-1 ring-green-500/50' : f === 'disagree' ? 'bg-red-500/30 text-red-400 ring-1 ring-red-500/50' : f === 'uncertain' ? 'bg-yellow-500/30 text-yellow-400 ring-1 ring-yellow-500/50' : 'bg-blue-500/30 text-blue-400 ring-1 ring-blue-500/50') : 'bg-slate-700 text-gray-400 hover:bg-slate-600'}`}>
              {f === 'all' ? `All (${results.length})` : `${f.charAt(0).toUpperCase() + f.slice(1)} (${counts[f]})`}
            </button>
          ))}
        </div>
      </div>
      <div className="grid grid-cols-12 gap-2 px-4 py-2 bg-slate-900/50 text-xs font-medium text-gray-500 uppercase border-b border-slate-700">
        <div className="col-span-1">#</div><div className="col-span-4">Question</div>
        <div className="col-span-2 text-center">User</div><div className="col-span-2 text-center">AI</div>
        <div className="col-span-1 text-center">Match</div><div className="col-span-1 text-center">Conf.</div><div className="col-span-1"></div>
      </div>
      <div className="divide-y divide-slate-700/50">
        {filtered.map((r, idx) => {
          ...
          return (
            <div key={key}>
              <div className={`grid grid-cols-12 gap-2 px-4 py-2.5 items-center cursor-pointer hover:bg-slate-700/30 ${r.match === 'disagree' ? 'bg-red-500/5' : r.match === 'uncertain' ? 'bg-yellow-500/5' : ''}`} onClick={() => toggle(key)}>
                <div className="col-span-1 text-sm font-bold text-blue-400">Q{r.questionNumber}</div>
                <div className="col-span-4 text-sm text-gray-300 truncate">{r.question}</div>
                <div className={`col-span-2 text-center text-sm font-semibold ${answerColor(r.userAnswer)}`}>{r.userAnswer}</div>
                <div className={`col-span-2 text-center text-sm font-semibold ${answerColor(r.aiAnswer)}`}>{r.aiAnswer}</div>
                <div className="col-span-1 flex justify-center"><MatchIcon match={r.match} /></div>
                <div className="col-span-1 flex justify-center"><ConfBadge c={r.confidence} /></div>
                <div className="col-span-1 flex justify-center">{open ? <ChevronUp className="w-4 h-4 text-gray-500" /> : <ChevronDown className="w-4 h-4 text-gray-500" />}</div>
              </div>
              {open && (
                <div className="px-4 pb-3 bg-slate-900/30 space-y-2 pt-2">
                  ...
                  {r.evidence && <div className="bg-slate-800/50 rounded p-2.5 border border-slate-700"><span className="text-xs text-gray-500 uppercase block mb-1">Evidence</span><p className="text-sm text-gray-300">{r.evidence}</p></div>}
                  {r.reasoning && <div className="bg-slate-800/50 rounded p-2.5 border border-slate-700"><span className="text-xs text-gray-500 uppercase block mb-1">Reasoning</span><p className="text-sm text-gray-300">{r.reasoning}</p></div>}
                  {r.details && <div className="bg-slate-800/50 rounded p-2.5 border border-slate-700"><span className="text-xs text-gray-500 uppercase block mb-1">Details</span><p className="text-sm text-gray-300 whitespace-pre-wrap">{r.details}</p></div>}
      ...
      <div className="divide-y divide-slate-700/50">
        {questions.map((q, i) => (
          <div key={i} className="px-4 py-2 flex items-start space-x-3">
            ...
            <span className={`text-sm font-medium flex-shrink-0 ${answerColor(q.userAnswer)}`}>{q.userAnswer}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function ChecklistComparison({ comparisonData }) {
  ...
  const download = () => {
    const r = { generatedAt: new Date().toISOString(), application: appDoc?.originalName || 'Unknown', standard: { summary: stdS, metadata: stdM, results: stdR }, programSpecific: { summary: psqS, results: psqR } }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-slate-800 rounded-lg p-5 border border-slate-700">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400">AI-derived answers vs user-provided answers from both checklists</p>
          ...
          <div className="flex items-center space-x-2">
            {hasResults && <button onClick={download} className="flex items-center space-x-1.5 bg-slate-700 hover:bg-slate-600 text-white px-3 py-2 rounded-lg text-sm transition-colors"><Download className="w-4 h-4" /><span>Export</span></button>}
      ...
      <div className="flex space-x-1 bg-slate-800 rounded-lg p-1 border border-slate-700">
        {[
          { id: 'standard', icon: ShieldCheck, label: 'Standard Q&A Comparison', summary: stdS },
          { id: 'program', icon: ClipboardList, label: 'Program-Specific Q&A Comparison', summary: psqS }
        ].map(t => (
          <button key={t.id} onClick={() => setSection(t.id)}
            ...
            {t.summary && <span className={`ml-1.5 px-1.5 py-0.5 rounded text-xs ${t.summary.agreementRate >= 80 ? 'bg-green-500/30 text-green-400' : t.summary.agreementRate >= 50 ? 'bg-yellow-500/30 text-yellow-400' : 'bg-red-500/30 text-red-400'}`}>{t.summary.agreementRate}%</span>}
          </button>
        ))}
      </div>
            ...
            : !stdL && stdQ && stdQ.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-medium text-gray-400">User-Provided Answers ({stdQ.length} questions)</h3>
        ...
        <div className="space-y-4">
          ...
          {psqR ? <ResultsTable results={psqR} filter={psqF} setFilter={setPsqF} expanded={psqE} toggle={k => setPsqE(p => ({ ...p, [k]: !p[k] }))} />
            : !psqL && psqQ && psqQ.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-medium text-gray-400">User-Provided Answers ({psqQ.length} questions)</h3>
